<script>
  ...
  import { allpackages } from "../../../Store/Company"
  ...
  const handleDelete = async () => {
    try {
      isExcuting = true
      const resDelete = await API.deletePackages(info.id)
...
<div class="body_wrapper">
  <div class="popup_wrapper">
    <div class="pop_header">
      <h5 class="mb-0 fw-bold">Delete package</h5>
      <span class="close_btn" on:click={() => dispatch("close")}>
        <Icon icon="icon-park-outline:close-one" width="28" />
      </span>
    </div>
    <div class="action_body">
      <span class="mt-3"><Icon icon="mingcute:delete-2-fill" width="50" style="color:crimson" /></span>
      <p class="fw-bold">Are you sure deleting this package?</p>
      <small>Note: This package will be delete on stripe </small>
    </div>
    <div class="action_bar">
      <CancelBtn title="Cancel" on:handleClick={() => dispatch("close")} />
      <Button title="Delete" on:handleClick={() => handleDelete()} excuting={isExcuting} />
    </div>
  </div>
</div>
