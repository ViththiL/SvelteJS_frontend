<script>
  ...
  const handleDelete = async () => {
    try {
      isExcuting = true
      const resDelete = await API.DeleteUser(info.id)
      if (resDelete.status === 200) {
        const resAllUsers = await API.getUsers()
        if (resAllPackages.status === 200) {
          allUsers.update(() => resAllUsers.data)
...
<div class="body_wrapper">
  <div class="popup_wrapper">
    <div class="pop_header">
      <h5 class="mb-0 fw-bold">Delete user</h5>
    ...
    <div class="action_body">
      <span class="mt-3"
        ...
        /></span
      >
      <p class="fw-bold">Are you sure deleting this user?</p>
    </div>
