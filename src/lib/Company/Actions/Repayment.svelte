<script>
  ...
  const handleRepayment = async () => {
    ...
    try {
      const resRefund = await API.customerRepayment(formData)
...
<div class="body_wrapper">
  <div class="popup_wrapper">
    <div class="pop_header">
      <h5 class="mb-0 fw-bold">Re-Pay</h5>
    ...
    <div class="form_wrapper ">
      ...
      <small>Are you sure about repay? </small>

      <div class="action_bar">
        <CancelBtn title="Cancel" on:handleClick={() => dispatch("close")} />
        <Button title="Re-Pay" on:handleClick={() => handleRepayment()} />
      </div>
    </div>
  </div>
</div>
